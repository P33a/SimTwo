...
def read():
    ...
    (x, x_dot, theta, theta_dot, x_target) = (None, None, None, None, None)
    ...
    theta = -float(datac)
    datad = data.split("d")[1].split("e")[0]
    theta_dot = -float(datad)
    datae = data.split("e")[1]
    x_target = -float(datae)
    return (x, x_dot, theta, theta_dot, x_target)
#A copy of:
"""
    Description:
...
class CartPole_SimTwo(gym.Env):
    ...
    def step(self, action):
        ...
        (x, x_dot, theta, theta_dot, x_target) = read()

        self.state = (x, x_dot, theta, theta_dot, x_target)
        ...
        if not done:
            reward = 1.0
            reward -= abs(x-x_target)/self.x_threshold
...
if __name__ == '__main__':
    ...
    #model.load("CartPole_position_1")

    model.learn(total_timesteps=10000000)

    model.save("CartPole_position_3_correct_reward")
