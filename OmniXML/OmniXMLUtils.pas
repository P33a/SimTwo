...
type
  ...
  {:SHA1 Hash}
  function SHA1HashFromBuffer(const Buf; const BufLength: Int64): TBytes;
  function SHA1HashFromStream(const Stream: TStream; ReadBytes: Int64 = -1): TBytes;
...
begin
  ...
  try
    xStream.WriteBuffer(Buf, BufLength);
    xStream.Position := 0;
    Result := SHA1HashFromStream(xStream);
  finally
    xStream.Free;
  end;
end;
...
function SHA1HashFromStream(const Stream: TStream; ReadBytes: Int64 = -1): TBytes;
  {$IFNDEF FPC}
  function SwapEndian(const AValue: Cardinal): Cardinal;
  begin
    Result :=
      ((AValue shl 24)) or
...
begin
  ...
  while x<>2 do
   begin
       ...
       begin
        ...
        if a<56 then
         begin
          ...
          d[14]:=SwapEndian(Cardinal(dl shr 32));
          d[15]:=SwapEndian(Cardinal(dl and $FFFFFFFF));
    ...
    while j<20 do
     begin
      a1:=((Int64(g[0]) shl 5) or (Int64(g[0]) shr 27))+
        ((Int64(g[1]) and Int64(g[2])) or (not Int64(g[1]) and Int64(g[3])))+
        Int64(g[4])+Int64(e[j])+Int64($5a827999);
      g[4]:=g[3];
      g[3]:=g[2];
    ...
    while j<40 do
     begin
      a1:=((Int64(g[0]) shl 5) or (Int64(g[0]) shr 27))+
        (Int64(g[1]) xor Int64(g[2]) xor Int64(g[3]))+
        Int64(g[4])+Int64(e[j])+Int64($6ed9eba1);
      g[4]:=g[3];
      g[3]:=g[2];
      g[2]:=((g[1] shl 30) or (g[1] shr 2));
      g[1]:=g[0];
    ...
    while j<60 do
     begin
      a1:=((Int64(g[0]) shl 5) or (Int64(g[0]) shr 27))+
        ((Int64(g[1]) and Int64(g[2])) or (Int64(g[1]) and Int64(g[3])) or (Int64(g[2]) and Int64(g[3])))+
        Int64(g[4])+Int64(e[j])+Int64($8f1bbcdc);
      g[4]:=g[3];
      g[3]:=g[2];
      g[2]:=((g[1] shl 30) or (g[1] shr 2));
      g[1]:=g[0];
    ...
    while j<80 do
     begin
      a1:=((Int64(g[0]) shl 5) or (Int64(g[0]) shr 27))+
        (Int64(g[1]) xor Int64(g[2]) xor Int64(g[3]))+
        Int64(g[4])+Int64(e[j])+Int64($ca62c1d6);
      g[4]:=g[3];
      g[3]:=g[2];
      g[2]:=((g[1] shl 30) or (g[1] shr 2));
      g[1]:=g[0];
    ...
    for j:=0 to 4 do
      h[j] := ((Int64(h[j]) + Int64(g[j])) and $FFFFFFFF);
   end;
  SetLength(Result,20);
  for j:=0 to 19 do
    Result[j] :=
      ((h[(j*2) shr 3] shr ((47{%H-}-(j*2)) shl 2) and $F) shl 4) or
       (h[(j*2+1) shr 3] shr ((47{%H-}-(j*2+1)) shl 2) and $F);
end;
